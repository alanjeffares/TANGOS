...
def get_path():
    ...
    results_dir = 'src/data_config.json'
    with open(results_dir) as f:
        results = json.load(f)
    return results['path_to_data']


def load_wine(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'winequality-red.csv')
    data = data[:1000]
...
def load_facebook(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'dataset_facebook.csv', sep=';')
...
def load_bioconcentration(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'Grisoni_et_al_2016_EnvInt88.csv', sep=',')
...
def load_student(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'student-por.csv', sep=';')
...
def load_skillcraft(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'SkillCraft1_Dataset.csv', sep=',')
...
def load_weather(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'Bias_correction_ucl.csv', sep=',')
    data = data.dropna()
    data = data[:1000]
...
def load_forest(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'forestfires.csv', sep=',')
...
def load_protein(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'CASP.csv', sep=',')
    data = data[:1000]
...
def load_heart(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'heart.dat', sep=' ', header=None)
...
def load_breast(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'breast-cancer-wisconsin.data', header=None)
...
def load_cervical(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'risk_factors_cervical_cancer.csv')
...
def load_credit(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + "crx.data", header = None)
    data = data[data[13] != '?']
...
def load_hcv(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + "hcvdat0.csv", index_col=0)
...
def load_tumor(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'primary-tumor.data', header=None)
    y = data[0] - 1
...
def load_soybean(seed, train_prop=0.8, batch_size=64):
    data0 = pd.read_csv(get_path() + 'soybean-large.data', header=None)
    data1 = pd.read_csv(get_path() + 'soybean-large.test', header=None)
...
def load_australian(seed, train_prop=0.8, batch_size=64):
    data = pd.read_csv(get_path() + 'australian.dat', sep=' ', header=None)
...
def load_entrance(seed, train_prop=0.8, batch_size=64):
    data_arff = arff.load(open(get_path() + 'CEE_DATA.arff'))
...
def load_thoracic(seed, train_prop=0.8, batch_size=64):
    data_arff = arff.load(open(get_path() + 'ThoraricSurgery.arff'))
    data = pd.DataFrame(data_arff['data'])
    y = data[16]
    y = y.replace({'T':1, 'F':0})
